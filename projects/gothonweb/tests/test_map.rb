...
class TestGame < Test::Unit::TestCase
  ...
  def test_gothon_game_map()
    assert_equal(Map::DEATH.name, Map::START.go('shoot!').name)
    assert_equal(Map::DEATH.name, Map::START.go('dodge!').name)

    room = Map::START.go('tell a joke')
    assert_equal(Map::LASER_WEAPON_ARMORY, room)

    # complete this test by making it play the game
    assert_equal(Map::DEATH.name, room.go('*').name)
    assert_equal(Map::THE_BRIDGE, room.go('0132'))
    
    room = room.go('0132')  # Enters The Bridge
    assert_equal(Map::THE_BRIDGE, room)
    
    assert_equal(Map::DEATH.name, room.go('throw the bomb').name)
    assert_equal(Map::ESCAPE_POD, room.go('slowly place the bomb'))
    
    room = room.go('slowly place the bomb')  # Enters Escape Pod
